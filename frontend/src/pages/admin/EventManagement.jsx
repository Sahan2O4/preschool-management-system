...
const EventManagement = () => {
  ...
  const fetchEvents = async () => {
    try { setLoading(true); const data = await eventAPI.getAll(); setEvents(data); }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        ...
        flash("✅ Event updated in database.");
        setEditId(null);
      } else {
        await eventAPI.create(form);
        flash("✅ Event created in database.");
      }
  ...
  return (
    <div className="admin-page">
      ...
      <div className="admin-body">
        ...
        {showForm && (
          <div className="form-card">
            ...
            <form onSubmit={handleSubmit} className="grid-form">
              <div className="form-group full"><label>Event Title *</label>
                <input required type="text" value={form.title} onChange={e => setForm({...form,title:e.target.value})} placeholder="Event name" /></div>
              ...
              <div className="form-group"><label>Location *</label>
                <input required type="text" value={form.location} onChange={e => setForm({...form,location:e.target.value})} placeholder="Venue" /></div>
              <div className="form-group"><label>Category</label>
                <select value={form.category} onChange={e => setForm({...form,category:e.target.value})}>{categories.map(c=><option key={c}>{c}</option>)}</select></div>
              <div className="form-group"><label>Organizer *</label>
                <input required type="text" value={form.organizer} onChange={e => setForm({...form,organizer:e.target.value})} placeholder="Organizer name" /></div>
              <div className="form-group full"><label>Description *</label>
                <textarea required rows="3" value={form.description} onChange={e => setForm({...form,description:e.target.value})} placeholder="Event details..." /></div>
        ...
        {loading ? <div className="loading-box">⏳ Loading events from database...</div> : (
          <div className="events-grid">
            {filtered.length === 0 ? (
              <p className="no-data">{events.length === 0 ? "No events yet. Add your first event above." : "No events in this category."}</p>
            ) : filtered.map(ev => (
              <div key={ev._id} className="event-card">
                <div className="ev-cat" style={{ background: categoryColors[ev.category]||"#aaa" }}>{ev.category}</div>
                <h3>{ev.title}</h3>
                <p className="ev-meta">📅 {new Date(ev.date).toLocaleDateString("en-LK",{weekday:"short",month:"long",day:"numeric",year:"numeric"})}</p>
                <p className="ev-meta">⏰ {ev.time} &nbsp;&nbsp; 📍 {ev.location}</p>
                <p className="ev-meta">👤 {ev.organizer}</p>
        ...
        {viewEvent && (
          <div className="modal-overlay" onClick={() => setViewEvent(null)}>
            <div className="modal-card" onClick={e => e.stopPropagation()}>
              ...
              <div className="ev-cat" style={{ background:categoryColors[viewEvent.category]||"#aaa", marginBottom:12 }}>{viewEvent.category}</div>
      ...
      <style>{`
        ...
        .event-card{background:white;border-radius:16px;padding:24px;box-shadow:0 4px 16px rgba(0,0,0,0.08);transition:transform 0.2s;display:flex;flex-direction:column;gap:8px;}
        .event-card:hover{transform:translateY(-5px);}
        .ev-cat{display:inline-block;padding:4px 14px;border-radius:20px;color:white;font-size:12px;font-weight:700;width:fit-content;}
        .event-card h3{font-size:18px;color:#333;font-weight:700;} .ev-meta{font-size:13px;color:#777;} .ev-desc{font-size:14px;color:#555;line-height:1.6;}
