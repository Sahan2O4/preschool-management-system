...
const SessionManagement = () => {
  ...
  const fetchSessions = async () => {
    try { setLoading(true); const data = await sessionAPI.getAll(); setSessions(data); }
    catch (err) { setError("Could not load sessions."); }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        await sessionAPI.update(editId, payload);
        flash("✅ Session updated in database.");
        setEditId(null);
      } else {
        await sessionAPI.create(payload);
        flash("✅ Session created in database.");
      }
  ...
  return (
    <div className="admin-page">
      <Navbar currentPage="Sessions" mode="admin" />
      <div className="admin-body">
        <div className="page-header">
          <div><h1>🎓 Session Management</h1><p>Create and manage interactive learning sessions</p></div>
          <button className="add-btn" onClick={() => { setShowForm(!showForm); setEditId(null); setForm(blank); }}>
        ...
        {showForm && (
          <div className="form-card">
            ...
            <form onSubmit={handleSubmit} className="grid-form">
              <div className="form-group full"><label>Session Title *</label>
                <input required type="text" value={form.title} onChange={e => setForm({...form,title:e.target.value})} placeholder="Session title" /></div>
              <div className="form-group"><label>Subject *</label>
                <select value={form.subject} onChange={e => setForm({...form,subject:e.target.value})}>{subjects.map(s=><option key={s}>{s}</option>)}</select></div>
              <div className="form-group"><label>Teacher *</label>
                <select value={form.teacher} onChange={e => setForm({...form,teacher:e.target.value})}>{teachers.map(t=><option key={t}>{t}</option>)}</select></div>
              ...
              <div className="form-group"><label>Time *</label>
                <input required type="time" value={form.time} onChange={e => setForm({...form,time:e.target.value})} /></div>
              <div className="form-group"><label>Duration (minutes) *</label>
                <input required type="number" min="15" value={form.duration} onChange={e => setForm({...form,duration:e.target.value})} /></div>
              <div className="form-group"><label>Level</label>
                <select value={form.level} onChange={e => setForm({...form,level:e.target.value})}>{levels.map(l=><option key={l}>{l}</option>)}</select></div>
              <div className="form-group"><label>Total Spots *</label>
                <input required type="number" min="1" value={form.spots} onChange={e => setForm({...form,spots:e.target.value})} /></div>
              <div className="form-group full"><label>Description *</label>
                <textarea required rows="3" value={form.description} onChange={e => setForm({...form,description:e.target.value})} placeholder="Session details..." /></div>
        ...
        <div className="filter-row">
          {["All",...subjects].map(s => (
            <button key={s} className={`filter-btn ${filterSubj===s?"active":""}`} onClick={() => setFilterSubj(s)}>{s}</button>
          ))}
        </div>

        {loading ? <div className="loading-box">⏳ Loading sessions from database...</div> : (
          <div className="sessions-grid">
            {filtered.length === 0 ? (
              <p className="no-data">{sessions.length===0?"No sessions yet. Add your first session above.":"No sessions in this subject."}</p>
            ) : filtered.map(s => {
              const isFull = s.enrolled >= s.spots;
              const pct = Math.round((s.enrolled/s.spots)*100);
              return (
                <div key={s._id} className="session-card">
                  <div className="sess-subj" style={{ background:subjectColors[s.subject]||"#aaa" }}>{s.subject}</div>
                  <h3>{s.title}</h3>
                  <p className="sess-meta">👩‍🏫 {s.teacher}</p>
                  ...
                  <div className="prog-bar"><div style={{ width:`${pct}%`, background:isFull?"#f87171":"linear-gradient(90deg,#4facfe,#ff7eb3)", height:"100%", borderRadius:"10px" }}/></div>
                  <p className="enrol-txt">{s.enrolled}/{s.spots} enrolled {isFull&&"• 🔴 Full"}</p>
        ...
        {viewSess && (
          <div className="modal-overlay" onClick={() => setViewSess(null)}>
            <div className="modal-card" onClick={e => e.stopPropagation()}>
              ...
              <div className="sess-subj" style={{ background:subjectColors[viewSess.subject]||"#aaa", marginBottom:12 }}>{viewSess.subject}</div>
      ...
      <style>{`
        ...
        .sessions-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:20px;}
        .session-card{background:white;border-radius:16px;padding:24px;box-shadow:0 4px 16px rgba(0,0,0,0.08);transition:transform 0.2s;display:flex;flex-direction:column;gap:8px;}
        .session-card:hover{transform:translateY(-4px);}
        .sess-subj{display:inline-block;padding:4px 14px;border-radius:20px;color:white;font-size:12px;font-weight:700;width:fit-content;}
        .session-card h3{font-size:17px;color:#333;font-weight:700;} .sess-meta{font-size:13px;color:#888;} .sess-desc{font-size:14px;color:#555;line-height:1.6;}
        .prog-bar{height:8px;background:#eee;border-radius:10px;overflow:hidden;margin-top:4px;}
        .enrol-txt{font-size:12px;color:#888;font-weight:600;}
